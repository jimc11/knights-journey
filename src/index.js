...
class Board extends React.Component {
  ...
  handleRefresh() {
    ...
    squaresCopy = Array(64).fill(null);
    this.setState({
      squares: squaresCopy,
    });
  }
  handleClick(i) {
    ...
    if (!this.startPicked && !this.endPicked) {
      ...
      const squaresCopy = this.state.squares.slice();
      squaresCopy[i] = (
        <img className="knight" src={knight} alt="knight" width="15px" height="24px" />
      );
      this.setState({
        squares: squaresCopy,
      });
  ...
  renderRefresh() {
    return <Refresh onClick={() => this.handleRefresh()} />;
  }
  r;
  findMin(src, dest) {
    let kt = new KnightTour();
    let d = new Dijkstra();
  ...
  render() {
    if (this.startPicked && this.endPicked)
      this.status =
        "Minimum moves for the journey: " + this.findMin(this.startSquare, this.endSquare);
    if (this.startPicked && !this.endPicked) this.status = "Choose an ending square:";
    if (!this.startPicked && !this.endPicked) this.status = "Choose a starting square:";
    ...
    return (
      <div>
        ...
        <div className="renderRefresh">{this.renderRefresh()}</div>
      </div>
    );
  }
}
...
// converts chessboard to a format that can be read by dijkstra's methods.  takes weighted adjacency matrix
// and converts to non-weighted adjacency matrix
// takes in 2d matrix, returns 2d matrix
...
class KnightTour {
  ...
  solveKT() {
    var ktGraph = []; // 64 sq w/ 8 possible moves

    // for loops to init
    for (let i = 0; i < 64; i++) {
      ktGraph[i] = [0, 0, 0, 0, 0, 0, 0, 0];
    }
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        nodeId = this.posToNodeId(row, col);
        newPositions = this.genLegalMoves(row, col);
        count = 0;
  ...
  //output is int array
  NodeIdToCoords(id) {
    let floor = Math.floor(id / 8);
    let mod = id % 8;
    return { floor, mod };
  }

  //output is single int
  posToNodeId(row, column) {
    return row * 8 + column;
  }

  //output is 2d int array
  genLegalMoves(x, y) {
    let newMoves = []; // 2d int arr, 8 x 2
    let temp = [0, 0];
    for (let i = 0; i < 8; i++) {
      newMoves[i] = temp;
    }
    ...
    for (let i = 0; i < 8; i++) {
      let newX = x + moveOffsets[i][0];
      let newY = y + moveOffsets[i][1];
      if (this.legalCoord(newX) && this.legalCoord(newY)) newMoves[i] = [newX, newY];
    }
    return newMoves;
  }
...
class Dijkstra {
  ...
  algo_dijkstra(graph, src_node) {
    ...
    let num_Vertices = 64;
    let path_array = []; // The output array. dist[i] will hold
    ...
    for (let i = 0; i < num_Vertices; i++) {
      // may be able to set this to 8 instead, for 8 possible connections per chess square
      path_array[i] = 1000;
      sptSet[i] = false;
    }
    ...
    for (let count = 0; count < num_Vertices; count++) {
      ...
      let u = this.minDistance(path_array, sptSet);
  ...
  minDistance(path_array, sptSet) {
    // Initialize min value
    let min = 1000;
    let min_index = -1;
    for (let v = 0; v < 64; v++) {
      // console.log(path_array[v] <= min);
